    ...
    <n-drawer v-model:show="showMobileMenu" :width="240" placement="right">
      <n-drawer-content style="padding: 0; background-color: #0d5a79;">
        <div style="display: flex; flex-direction: column; height: 100%;">
          ...
          <n-menu :options="topMenuOptions" :theme-overrides="menuTheme" :value="activeKey" @update:value="handleMenuClick" />
          <div style="flex-grow: 1;"></div>
          <n-menu :options="bottomMenuOptions" :theme-overrides="menuTheme" :value="activeKey" @update:value="handleMenuClick" />
        </div>
      </n-drawer-content>
    </n-drawer>
    ...
    <n-layout has-sider style="height: 100%;">
      <n-layout-sider class="desktop-sider" bordered show-trigger collapse-mode="width" :collapsed-width="64" :width="240" style="height: 100vh; overflow-y: unset; position: sticky; top: 0;">
        <div style="display: flex; flex-direction: column; height: 100%;">
          ...
          <n-menu :options="topMenuOptions" :theme-overrides="menuTheme" :collapsed-icon-size="25" class="custom-collapsed-menu" :value="activeKey" @update:value="handleMenuClick" />
          <div style="flex-grow: 1;"></div>
          <n-menu :options="bottomMenuOptions" :theme-overrides="menuTheme" :collapsed-icon-size="25" class="custom-collapsed-menu" :value="activeKey" @update:value="handleMenuClick" />
        </div>
      </n-layout-sider>
...
import { BookFilled, LogOutFilled, PersonFilled, MenuFilled } from "@vicons/material";
...
export default defineComponent({
  ...
  setup() {
    ...
    const getActiveKeyFromPath = (path) => {
      ...
      if (parts.length <= 1 || (parts.length === 2 && parts[1] === "admin")) return "dashboard";

      const mainRoutes = ["dashboard", "courses", "profile"];
      const currentMainRoute = parts[1];

      if (mainRoutes.includes(currentMainRoute)) return currentMainRoute;

      const matchedRoute = router.currentRoute.value.matched.find((route) =>
        mainRoutes.some((mainRoute) => route.path.includes(mainRoute))
      );

      if (matchedRoute) {
        const matchedMainRoute = matchedRoute.path.split("/").find((part) =>
          mainRoutes.includes(part)
        );
